...
tldr: 针对多步生成式策略在机器人控制中推理速度慢、难以进行高频闭环控制和在线强化学习的问题，本文提出了一种原生单步生成式策略框架。该框架包含漂移策略（DBP）和漂移策略优化（DBPO），通过将迭代优化过程内化到模型参数中，实现了在保持多模态建模能力的同时，将推理速度提升达100倍。实验证明该方法在离线模仿学习、在线微调及真实机器人高频控制任务中均表现优异，为高效实时控制提供了新途径。
source: arxiv
...
motivation: 现有的多步生成式策略因推理时需要多次迭代计算，导致其在处理高频闭环控制和在线强化学习时效率低下且成本高昂。
method: 提出漂移策略（DBP）利用固定点漂移目标实现单步生成，并开发DBPO框架以支持在保持单步部署特性的同时进行稳定的在线强化学习。
result: 该方法在多个操纵基准测试中匹配或超越了扩散策略的性能，推理速度提升达100倍，并在真实双臂机器人上实现了105.2 Hz的高频控制。
conclusion: 本研究通过将细化过程从推理阶段转移至训练阶段，成功构建了兼具高性能与高效率的原生单步生成式机器人控制策略。
---
